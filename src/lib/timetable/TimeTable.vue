...
const api = {
  ...
  removeEntry: (day, timeSlot, classId) => {
    const newTimetable = props.timetable.filter(item => 
      !(item.day === day && 
...
<template>
  <div class="w-full overflow-hidden">
    <div class="overflow-auto max-w-full border border-gray-200 rounded-lg">
      <table class="w-full border-collapse table-fixed">
        <!-- Header Row with Days -->
        <thead>
          <tr>
            <th class="w-32 p-2 bg-gray-50 border border-gray-200"></th>
            <th 
              v-for="day in days" 
              :key="day" 
              :colspan="timeSlots.length" 
              class="p-2 bg-gray-50 border border-gray-200 text-center font-semibold"
            >
              {{ day }}
            </th>
          </tr>
          ...
          <tr v-if="showTimeLabels">
            ...
            <template v-for="day in days" :key="day">
              <th
                ...
                :key="slot.id"
                class="p-2 bg-gray-50 border border-gray-200 text-xs text-gray-600 font-medium"
              >
                {{ slot.label }}
              </th>
            </template>
        ...
        <!-- Classes Rows -->
        <tbody>
          <tr v-for="(className, index) in classes" :key="index" class="hover:bg-gray-50">
            <!-- Class Name Column -->
            <td class="w-32 p-3 border border-gray-200 font-medium">
              <div class="truncate">
                {{ className }}
              </div>
            </td>
            ...
            <template v-for="day in days" :key="day">
              <td
                ...
                class="w-24 p-1 border border-gray-200 transition-colors"
                :class="{
                  'hover:bg-blue-50 cursor-pointer': editable,
                  'bg-gray-50': !editable
                }"
              ...
              >
                <div 
                  ...
                  class="h-full p-1 rounded border-l-2"
                  :style="{
                ...
                >
                  ...
                  <div v-if="getAssignment(day, slot.id, index).teacher" class="text-xs text-gray-600 truncate">
                    {{ getAssignment(day, slot.id, index).teacher }}
                  </div>
                </div>
                <div v-else-if="editable" class="h-full flex items-center justify-center text-gray-400 hover:text-blue-500">
